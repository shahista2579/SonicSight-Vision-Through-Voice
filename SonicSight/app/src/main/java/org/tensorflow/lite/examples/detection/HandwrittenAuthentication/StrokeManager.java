...
import static org.tensorflow.lite.examples.detection.HandwrittenAuthentication.DigitalInkMainActivity.drawingView;
import static org.tensorflow.lite.examples.detection.HandwrittenAuthentication.DigitalInkMainActivity.textToSpeech;
import static org.tensorflow.lite.examples.detection.HandwrittenAuthentication.DigitalInkMainActivity.userintt;
...
public class StrokeManager {
    ...
    public interface PinRecognizedListener {
        void onPinRecognized(String pin);
    }

    @Nullable
    private PinRecognizedListener pinRecognizedListener = null;

    public void setPinRecognizedListener(PinRecognizedListener listener) {
        this.pinRecognizedListener = listener;
    }
...
/**In the current use this statement is never reached because we only ever send
 ...
 // This line is necessary because otherwise Java's static analysis doesn't allow for compiling. Returning false indicates that a message wasn't handled.*/
    ...
    private void commitResult() {
        ...
        if (!recognitionTask.done() || recognitionTask.result() == null) return;
        ...
        num = num.replace("o", "0").replace("g", "9").replace("I", "1").replace("l", "1").replaceAll("[^0-9]", "");

        boolean isPinMode = userintt == 0;
        boolean isPasswordMode = userintt == 2;

        if (num.isEmpty()) {
            handleRecognitionError(isPinMode, isPasswordMode);
        } else if (isPasswordMode && list1.size() < 6) {
            list1.add(num);
            setStatus1(String.valueOf(list1.size()));
            count = list1.size();
            speakPasswordPrompt(count);
        ...
        } else if (isPinMode && Pin.size() < 4) {
            Pin.add(num);
            setStatus1(String.valueOf(Pin.size()));
            pincount = Pin.size();
            speakPinPrompt(pincount);
            //DigitalInkMainActivity.toastString("pin " + Pin.toString());
            if (Pin.size() == 4) {
                isClearCurrentInkAfterRecognition = false;
                DigitalInkMainActivity.toastString(getPin().toString());
                if (pinRecognizedListener != null) {
                    StringBuilder pinBuilder = new StringBuilder();
                    for (String digit : Pin) pinBuilder.append(digit);
                    pinRecognizedListener.onPinRecognized(pinBuilder.toString());
                }
            }
        }
    ...
    private void handleRecognitionError(boolean isPinMode, boolean isPasswordMode) {
        if (isPasswordMode && list1.size() == 6) {
            textToSpeech.stop();
    ...
    private void speakPasswordPrompt(int count) {
        switch (count) {
            case 1:
    ...
    private void speakPinPrompt(int pincount) {
        switch (pincount) {
            case 1:
